...
3. **Capa Decay (Weight Decay):** Si se utiliza Decay en el modelo (`mlpKerasDecay`), se incorpora regularización L2 mediante el parámetro `lambda`. La regularización L2 penaliza los pesos grandes, evitando así el sobreajuste y mejorando la generalización del modelo. Nosotras utilizaremos dropout o decay por separado para comparar el rendimiento.
...
Aqui volvemos a hacer una particion del conjunto.
```{r}
set.seed(125)
...
comparacion
```
Al contrario que en arboles, utilizando min-max, podemos ver que aqui si que se tiene en cuenta la posición en el ranking y los movimientos. Con el modelo con el que mejores resultados obtenemos es con el mlpNorm1Decay_model. Por lo que, conseguimos que nos funcionen mejor las redes utilizando Decay que Dropout.
/TO DO TODO PLOT
...
Pues los resultados obtenidos anteriormente tambien se mantienen. El mejor modelo es el obtenido con Decay y que contiene los parametros de ranking y movimientos en la lista.
...
```
```{r knnRegZ}
knnRegNoMov_model = readRDS("knn/knnRegZ.rds")
knnRegNoMov_model
...
```{r, eval=TRUE}
rf_200t = readRDS("classification200.rds")
rf_300t = readRDS("classification300.rds")
rf_400t = readRDS("classification400.rds")
rf_500t = readRDS("classification500.rds")
...
```{r}
readRDS("spotifyNoMovementsClasificacion.rds")
```
