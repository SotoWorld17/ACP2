...
A continuacion se puede ver como hemos generado un grid y ademas hemos probado con distintos numeros de arboles. Para 200,300,400 y 500 y seguidamente veremos si el numero de arboles es significativo.

En este chunk , se puede ver además comentado, que antes de ejecutar el codigo con todos los datos, se hizo tambien para un porcentaje de datos de la muestra. En nuestro caso fue del 30%. Al encontrar resultados significativos con el grid pensado, hemos decidido seguir adelante con él y más abajo se contrastaran los resultados y se probara que al menos una de las configuraciones nos produce unos resultados aceptables. 
...
```{r}
predicciones <- predict(rf_200t, newdata = testing)
...
```{r}
predicciones <- predict(rf_300t, newdata = testing)
mae <- mae(predicciones, testing$popularity)
rmse <- rmse(predicciones, testing$popularity)
...
Todos estos requisitos han sido cumplidos gracias al proceso de preparación de datos que hemos llevado a cabo.
...
modelo <- keras_model_sequential() %>%
  layer_dense(units = 64, activation = "relu", input_shape = length(spotiRNN.Vars.Entrada.Usadas)) %>%
  layer_dense(units = 1)
