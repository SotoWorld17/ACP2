...
porcentaje_datos <- 1
savePoint=spotify
tamanio_muestra <- round(nrow(savePoint) * porcentaje_datos)
savePoint = savePoint[sample(nrow(savePoint), tamanio_muestra), ]


set.seed(123)

inTraining <- createDataPartition(savePoint$popularity, times=1,p = .80, list = FALSE)
training <- savePoint[ inTraining,]
testing  <- savePoint[-inTraining,]
...
rpartFit <- train(popularity ~ ., data = training, 
                 method = "ranger", 
...
```{r}
predicciones <- predict(rpartFit, newdata = testing)
...
set.seed(123)
inTraining <- createDataPartition(spotify$popularity, times=1,p = .75, list = FALSE)
...
```{r}
predicciones <- predict(spotifyrf, newdata = testing)
mae <- mae(predicciones, testing$popularity)
rmse <- rmse(predicciones, testing$popularity)
