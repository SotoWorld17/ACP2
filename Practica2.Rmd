...
Hasta ahora hemos borrado los parametros que nos parecían más evidentes, pero pensando en la resolución del problema, nosotras tambien llegamos a la conclusión de que los parámetros `daily_movement`, `daily_rank` y `weekly_movement` a pesar de que nos aportarán algo y sabemos que estan relacionados con la popularidad, no nos ayudarán a determinar si una canción cualquiera se va a hacer famosa (al igual que con el parámetro `artists`). Hemos llegado a esta conclusión debido a que el pensamiento que tenemos nosotras al hacer esta practica es que vamos a crear un proyecto para determinar si una cancion será popular o no, y estos tres parametros nos dan informacion sobre canciones en la lista pero nada más. Para confirmar nuestras sospechas trabajaremos con un dataset conjunto que si que tenga estos tres parametros, y despues comprobaremos la precisión. Nuestras conclusiones previas es que sobre estos parametros si que cae cierta importancia y que puestos más altos en el daily rank supondran una mayor popularidad pero creemos que no nos servira de mucho a la hora de determinar la posible popularidad de una canción futura.
```{r}
...
```{r}
#SAVE POINT PARA CUANDO LA CAGO. BORRAR 
savePoint = spotify
```
...
Los predictores que habría que normalizar son principalmente las características de las canción, para ello se podría utilizar el método min-max o la estandarización, pensando de cara a la próxima parte de la práctica y sabiendo de la existencia de valores negativos, creemos que es mejor hacerlo mediante el z-score para ello vamos a utilizar la función `scale` disponible en R.

```{r}
songFeatures <-  c('danceability', 'energy', 'loudness', 'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo', 'duration_ms', 'diferencia_dias')
```
...
Además, creemos que sería interesante escalar `daily_movement`, `weekly_movement`, `between_week_prev_popularity`, `day_prev_popularity` y, como `daily_rank` tiene relación con las otras también la vamos a escalar.

```{r}
masDatos <- c('daily_movement', 'weekly_movement', 'daily_rank', 'between_week_prev_popularity', 'day_prev_popularity') 
...
Sí, lo hemos hecho anteriormente combinando las variables `album_release_date` y `snapshot_date` dando lugar a `diferencia_dias` y hemos añadido los atributos `between_week_prev_popularity` y `day_prev_popularity` para tener guardados los datos de la popularidad previa.
...
spotiNormalizado$velocidad_popularidad <- 1 / (spotify$diferencia_dias + epsilon)
```
...
fitControl <- trainControl(## 100-fold CV
                           method = "cv",
                           number = 10)
...
inTraining <- createDataPartition(spotify$spotifyNoMovements, times=1,p = .80, list = FALSE)
...
Tras ver los resultados del apartado anterior, vamos a utilizar los siguientes hiperparámetros:
...
```{r}
spotiNormalizado[, 'velocidad_popularidad'] <- lapply(spotiNormalizado[, 'velocidad_popularidad'], min_max_normalize)
```
...
spotiRNN.Var.Salida.Usada <- c("popularity")
spotiRNN.Vars.Entrada.Usadas <- setdiff(names(spotiNormalizado), spotiRNN.Var.Salida.Usada)

# Crear una partición del 80% para entrenamiento y 20% para prueba
spotiRNN.Particion <- createDataPartition(spotiNormalizado[[spotiRNN.Var.Salida.Usada]], 
                                         p = 0.8, 
                                         list = FALSE)

# Crear conjuntos de entrenamiento y prueba
spotiRNN.Datos.Entrenamiento <- spotiNormalizado[spotiRNN.Particion, ]
spotiRNN.Datos.Prueba <- spotiNormalizado[-spotiRNN.Particion, ]

# Configurar la red neuronal
modelo <- keras_model_sequential() 
modelo %>%
  layer_dense(units = 64, input_shape = length(spotiRNN.Vars.Entrada.Usadas)) %>%
...
historial_entrenamiento <- modelo %>% fit(
  as.matrix(spotiRNN.Datos.Entrenamiento[, spotiRNN.Vars.Entrada.Usadas]),
  spotiRNN.Datos.Entrenamiento[[spotiRNN.Var.Salida.Usada]],
  epochs = 10,
  batch_size = 32,
...
system.time(
  spotiKNN.modelo.knn <- train(x = spotiKNN.Datos.Entrenamiento[, spotiKNN.Vars.Entrada.Usadas],
                               y = spotiKNN.Datos.Entrenamiento[[spotiKNN.Var.Salida.Usada]],
                               method = 'knn')
)
```
### Descripción de hiperparámetros
